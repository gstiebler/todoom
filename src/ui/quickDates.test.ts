...
describe('quickDates', () => {
  it('offers today, tomorrow, the coming weekend and next Monday', () => {
    expect(quickDates(TODAY).map((q) => [q.label, q.date])).toEqual([
      ['Today', '2026-09-10'],
      ['Tomorrow', '2026-09-11'],
  ...
  it('keeps this weekend on the same day when today is Saturday', () => {
    const weekend = quickDates('2026-09-12').find((q) => q.key === 'weekend')
    expect(weekend?.date).toBe('2026-09-12')
  })

  it('always moves next week forward, never to today', () => {
    const monday = quickDates('2026-09-14').find((q) => q.key === 'next-week')
...
describe('monthGrid', () => {
  it('pads the first row so the 1st lands under its weekday', () => {
    ...
    expect(grid.title).toBe('Sep 2026')
  ...
  it('handles a leap February', () => {
    expect(monthGrid(2028, 2).cells.at(-1)).toBe('2028-02-29')
  })
})
